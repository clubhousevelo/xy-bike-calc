class SeatpostCalculator {
    ...
    initializeEventListeners() {
        ...
        window.addEventListener('resize', () => {
            if (this.canvas) {
                ...
                // Redraw current state by recalculating values
                ...
                if (saddleX && saddleY) {
                    // Recalculate values instead of reading from DOM
                    let bbToRail = '--';
                    let exposedSeatpost = '--';
                    
                    if (seatTubeAngle) {
                        // Calculate BB to Rail
    ...
    calculateResults() {
        ...
        if (saddleX && saddleY) {
            // Effective STA calculation (always calculated when X and Y are provided)
            const angleFromVertical = Math.atan2(saddleX, saddleY) * 180 / Math.PI;
            effectiveSTA = (90 - angleFromVertical).toFixed(1);
            
            // BB to Rail calculation - follows seat tube angle to saddle Y position
            if (seatTubeAngle) {
                // Calculate the distance along the seat tube angle to reach saddle Y
                const seatTubeLengthToSaddleY = saddleY / Math.sin((90 - seatTubeAngle + 90) * Math.PI / 180);
                bbToRail = Math.round(seatTubeLengthToSaddleY);
            } else {
                // Fallback to direct distance if no seat tube angle provided
                const dx = saddleX;
                const dy = saddleY;
    ...
    updateVisualization(saddleX, saddleY, seatTubeAngle, seatTubeLength, bbToRail, exposedSeatpost, effectiveSTA) {
        ...
        const centerX = canvas.width * 0.99;
        ...
        // Origin point (bottom bracket) - positioned to center the visualization
        ...
        const seatTubeLengthToSaddleY = saddleY / Math.sin((180 - seatTubeAngle) * Math.PI / 180);
        const displaySeatTubeLength = seatTubeLength || seatTubeLengthToSaddleY; // Use calculated length as default if not provided
        ...
        // BB label
        ctx.fillText('BB', originX + 25, originY + 4);
